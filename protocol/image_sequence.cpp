...
void ImageSequence::run(GpioHal *hal, ICamera *camera, const std::optional<size_t> &exposure_time_ms_override, const std::optional<std::string> &output_dir, std::function<bool()> &interrupt_requested, const boost::asio::ip::address &peer_address) const
{
    ...
    do
    {
        for (size_t image_index = 0; image_index < images_.size(); ++image_index)
        {
            ...
            // This early return can only happen now to ensure all calls to change_filter_async are matched with wait_for_filter_move
            if (interrupt_requested())
            {
                return;
            }

            auto shutter_callback = [this, &hal, &params, &image_index, &peer_address]
            ...
            if (live_preview_)
            {
                ...
                std::this_thread::sleep_for(std::chrono::duration<unsigned int, std::milli>(params.longest_flash_duration_ms * 2));
            }
        }
    }
    while(live_preview_);
